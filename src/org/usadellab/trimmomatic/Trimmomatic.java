...
public class Trimmomatic {
	...
	static int calcAutoThreadCount() {
		...
		if(cpus <= MAX_AUTO_THREADS_THRESHOLD) {
            return Math.min(cpus, MAX_AUTO_THREADS_ALLOC);
        }
	...
	static Trimmer[] createTrimmers(Logger logger, Iterator<String> nonOptionArgsIter) throws IOException {
		...
		while(nonOptionArgsIter.hasNext()) trimmerList.add(fac.makeTrimmer(nonOptionArgsIter.next()));
	...
	public static void main(String[] args) throws Exception {
		...
		if(args.length > 0) {
			...
			if(mode.equals("PE")) {
				if(TrimmomaticPE.run(restOfArgs)) showUsage = false;
			} else if(mode.equals("SE")) {
				if(TrimmomaticSE.run(restOfArgs)) showUsage = false;
