...
public class TrimmomaticPE extends Trimmomatic {
	...
	public void processPipeline(FastqParser rawParser1, FastqParser rawParser2,
								File output1P,File output1U, File output2P, File output2U,
								Trimmer trimmers[], File trimLog, File statsSummary, PairingValidator pairingValidator,
		...
		while (!done) {
            ...
            while (recs1 == null) recs1 = parser1.poll();
            while (recs2 == null) recs2 = parser2.poll();
			...
			if (pairingValidator != null) pairingValidator.validatePairs(recs1, recs2);
			...
			if (logCollector != null) logCollector.put(future);
		}
	...
	public void process(File input1, File input2, File output1P, File output1U, File output2P, File output2U,
		...
		if(validatePairing) pairingValidator = new PairingValidator(logger);
	...
	private static File[] calculateTemplatedInput(String baseStr) {
		...
		for (String[] pair : translation) {
			...
			if (tmp != null) return new File[] {fileBase, new File(baseDir, tmp+exts)};
		}
	...
	private static File[] calculateTemplatedOutput(String baseStr) {
		...
		return new File[] {
				new File(baseDir,core + "_1P" + exts),
				new File(baseDir,core + "_1U" + exts),
				new File(baseDir,core + "_2P" + exts),
				new File(baseDir,core + "_2U" + exts)
		};
	}
	...
	public static boolean run(String[] args) throws Exception {
		...
		while (argIndex < args.length) {
			...
			if (arg.startsWith("-")) {
				if (arg.equals("-phred33")) phredOffset = 33;
				...
				else if (arg.equals("-threads")) threads = Integer.parseInt(args[argIndex++]);
					...
					if (argIndex < args.length) templateInput = args[argIndex++];
					else badOption = true;
				}
				else if (arg.equals("-baseout")) {
					if (argIndex < args.length) templateOutput = args[argIndex++];
					else badOption = true;
				}
				else if (arg.equals("-validatePairs")) validatePairs = true;
		...
		if ((nonOptionArgs.size() < additionalArgs) || badOption) return showVersion;
		...
		if(threads == 0) {
			...
			if(threads > 1) logger.infoln("Multiple cores found: Using " + threads + " threads");
		}
	...
	public static void main(String[] args) throws Exception {
		if (!run(args)) {
			System.err.println("Usage: [-version] [-threads <threads>] [-phred33|-phred64] [-trimlog <trimLogFile>] " +
					"[-summary <statsSummaryFile>] [-quiet] [-validatePairs] [-compressLevel <lvl>] " +
					"[-compressStream|-compressBlock] [-basein <inputBase> | <inputFile1> <inputFile2>] " +
